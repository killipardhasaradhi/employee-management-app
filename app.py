...
# STAGE 1 & 2: LOGIN VIA OTP (UNCHANGED)
...
# STAGE 3: LOGGED IN PORTAL
    ...
    # =========================================================
    # 🏢 2. HOST & EMPLOYEE MODES
    # =========================================================
    else:
        role = st.radio("Select Portal Mode", ["Company Host / Admin", "Employee Portal"], horizontal=True)

        # -----------------------------------------------------
        # COMPANY HOST / ADMIN DASHBOARD
        # -----------------------------------------------------
        if role == "Company Host / Admin":
            ...
            if host_check:
                ...
                # BEAUTIFUL COLORFUL COMPANY HEADER FOR HOST
                st.markdown(f'<div class="company-header">🏢 {comp_name}</div>', unsafe_allow_html=True)
                st.success(f"Welcome Host: **{company_info.get('host_name')}**")
                ...
                if host_menu == "📊 Today's Attendance Summary":
                    ...
                    all_emps = supabase.table("employees").select("*").eq("business_name", comp_name).execute().data or []
        ...
        # -----------------------------------------------------
            ...
            if emp_records:
                emp = emp_records[0]
                comp_name = emp.get("business_name")
                emp_name = emp.get("name")
                
                # BEAUTIFUL COLORFUL COMPANY HEADER FOR EMPLOYEE
                st.markdown(f'<div class="company-header">🏢 {comp_name}</div>', unsafe_allow_html=True)
                    ...
                    if submit_new:
                        if not name.strip() or not biz_input:
                            ...
                            try:
                                ...
                                st.success(f"Profile saved under '{biz_input}'!")
                                st.rerun()
                            except Exception as e:
